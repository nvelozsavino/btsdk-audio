...
extern void sha256_update(sha256_context_t* context, UINT8* input, UINT32 input_len);
extern void sha256_final(sha256_context_t* context, UINT32* output);
...
extern void ECC_PM_B_NAF(Point *q, Point *p, uint32_t *n, UINT32 keyLength);
#else
...
void fastpair_sec_sha256(uint8_t *p_in, uint16_t in_len, uint8_t *p_out)
{
    ...
    sha256_final(&context, (UINT32*)p_out);
...
int fastpair_sec_uecc_shared_secret(uint8_t *peer_pub_key, uint8_t *priv_key, uint8_t *secret)
{
    ...
    WICED_BT_TRACE("run 43012 ROM secret\n");

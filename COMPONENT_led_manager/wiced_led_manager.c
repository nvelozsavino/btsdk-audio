...
wiced_result_t wiced_led_manager_init( wiced_led_config_t* config )
{
	...
	if(WICED_SUCCESS !=	platform_led_init( &platform_led_config[config->led], LED_FREQ, config->bright))
		return WICED_ERROR;

	led_timer[config->led].led = config->led;

    /* initialize timer */
    if (wiced_init_timer(&led_timer[config->led].timer,
                &led_timer_function,
                (uint32_t)config->led,
                WICED_MILLI_SECONDS_PERIODIC_TIMER) != WICED_SUCCESS)
    {
        WICED_BT_TRACE("LED init timer failed\n");
        platform_led_deinit(&platform_led_config[config->led]);
        return WICED_ERROR;
    }
...
wiced_result_t wiced_led_manager_reconfig_led( wiced_led_config_t* config)
{
	...
	return platform_led_reinit( &platform_led_config[config->led], LED_FREQ, config->bright);
}
...
wiced_result_t wiced_led_manager_blink_led(wiced_led_t led, uint32_t on_period, uint32_t off_period)
{
	...
	if(WICED_SUCCESS != wiced_start_timer(&led_timer[led].timer, on_period))
	{
		WICED_BT_TRACE("LED start timer failed\n");
		return WICED_ERROR;
	}
